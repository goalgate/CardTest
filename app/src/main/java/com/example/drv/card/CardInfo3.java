...
//身份证读取类
public class CardInfo3 extends SerialPortCom implements ICardInfo {
    ...
    public CardInfo3(String port, ICardState iCardState) {
        if (!port.equals(""))
            setDevName(port);
    ...
    public boolean isICData(byte[] b1, byte[] b2) {
        if (b1 == null || b2 == null) {
            return false;
        }
        if (b1.length < b2.length) {
    ...
    public boolean readIC(final byte[] bs) {
        if (cmd_ic_type == 0) {
            ...
            //            if(bs[0] ==0x02&&bs[1]==0x02&&bs[2]==0x08){
            //                try {
            //                    Thread.sleep(200);
            //                }catch (Exception e){
            //
            ...
            if (isData(bs, dt_ica_readOK)) {
                ...
                byte[] bx = new byte[4];
                Lg.e("ic_bs", byteToStr(bs, bs.length));
                System.arraycopy(bs, 7, bx, 0, 4);
                uid_ = byteToStr(bx, 4).toUpperCase();
                Lg.e("ic_uid", uid_);
